...
\matrix [column sep=10mm,row sep=8mm] {
  ...
  \node [block, green] (h) {Remove all contours which are not direct subcontours of the coarse table contour};
      & \node [shortblock, yellow] (ra) {Insert missing cell contours into main contours}; \\
...
\begin{scope}[every path/.style=line]
  ...
  \path[secondpass, rounded corners=5pt] (ra) |- (e);
  % Second pass
  \path[secondpass] ([xshift=5mm]e.south) -- ([xshift=5mm]f.north);
  \path[secondpass] ([xshift=5mm]f.south) -- ([xshift=5mm]g.north);
  \path[secondpass] ([xshift=5mm]g.south) -- ([xshift=5mm]h.north);
  \path[secondpass] ([xshift=5mm]h.south) -- ([xshift=5mm]i.north);
